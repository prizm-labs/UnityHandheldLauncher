...
namespace Prizm {
	...
	public class CardPiece : Piece {
		...
		public CardPrefabData myPrefabData;
		...
		protected override void OnEnable() {
			...
			//by default, all card pieces play a sound and are enlarged
			StartMoveMacro = StartMovingPiece;

			//by default, all card pieces snap to their hosted zone on release
			StopMoveMacro = StopMovingPiece;

			ReleaseMacro = SnapBackPiece;
		...
		public void FlipMeOver() {
			transform.localEulerAngles = transform.localEulerAngles + Vector3.forward * 180;
		}
		...
		{
			if (myPrefabData == null) {
				myPrefabData = new CardPrefabData ();
			}
		...
		protected void StopMovingPiece() {
			...
			if (rubberBanding && myHostZone != null) {
				...
				//Debug.LogWarning("not rubber banding card!: " + name);
			}
		}

		protected void SnapBackPiece() {
